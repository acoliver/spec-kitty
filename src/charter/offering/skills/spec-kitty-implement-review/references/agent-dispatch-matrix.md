...
| LLxprt Code | `llxprt` | `cd <dir> && llxprt -p "<prompt>" --approval-mode=yolo` | Bun-based Gemini CLI fork. No `-C` and no `--output-format json`; `cd` into the workspace first. Model profiles via `--profile-load <name>`. |
